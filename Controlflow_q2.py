# Question 2: Provide a list with the name(s) of the director(s) with the most Oscar wins. We are asking for a list 
# because there could be more than 1 director tied for the most Oscar wins. 
...
win_count_dict = {}
for win_year, win_list in winners.items():
    for win in win_list:
        win_count_dict[win] = win_count_dict.get(win, 0) + 1
print(win_count_dict, '\n')

most_wins = max(win_count_dict.values())
print('The most wins by a director is: ', most_wins, 'wins', '\n')

most_win_director = [key for key, value in win_count_dict.items() if value == most_wins]
