...
# the numbers are prime, the code should print "[number] is a prime number." If the number is NOT a prime number, 
# it should print "[number] is not a prime number", and a factor of that number, other than 1 and the number itself: 
# "[factor] is a factor of [number]". Example output: 
...
for val in check_prime:
    for x in range(2, val):
        if (val % x) == 0:
            print('{} is NOT a prime number, because {} is a factor of {}'.format(val, x, val))
            break
        if x == val -1:
